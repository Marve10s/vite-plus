...
For a Homebrew installation, the first `vp` command sets up your shell, shims, and environment-management preferences. It reuses Homebrew's binary and any bundled JavaScript. A native-only formula installs matching JavaScript dependencies in your data directory. Setup stores its completion state in your user directories and does not need write access to the Homebrew prefix.
